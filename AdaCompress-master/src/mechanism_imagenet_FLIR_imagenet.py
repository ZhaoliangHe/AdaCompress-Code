...
# ref_cache = defaultdict(dict)
# with open('evaluation_results/image_reference_cache_face_inference.defaultdict', 'rb') as f:
#     ref_cache = pickle.load(f)
# with open('evaluation_results/image_reference_cache_face_retrain_DNIM.defaultdict', 'rb') as f:
#     ref_cache = pickle.load(f)
...
subset = []
for time_stemp in os.listdir("/home/DNIM/DNIM/time_stamp/"):
    for line in fileinput.input("/home/DNIM/DNIM/time_stamp/" + time_stemp):
        name, date, hour, m = line.strip().split(" ")
        if 0 <= int(hour) < 6 or int(hour) > 19:
            date = name.split("_")[0]
            img_id = name.split("_")[1].split(".")[0]
            subset.append("/home/DNIM/DNIM/Image/sorted_by_time/%s_%s_%s.jpg" % (
...
class RunningAgent(object):
    ...
    def agent_upload(self, image_path):
        ...
        ref_labels = np.array([line['keyword'] for line in ref_results])[
        ...
        if self.STATUS == "INFERENCE":
            ...
            if len(self.agent_memory['recent_accuracy']) > self.recent_zone and recent_acc < self.acc_threshold:
                ...
                if (self.flag == 'imagenet' and self.step_count > 2000) or (
                        self.flag == 'FLIR' and self.step_count > 5000):
                    print("yes")
                    if (self.load_model == 1 and self.step_count > 5000):
                        print("load model\n")
                        self.STATUS = "INFERENCE"
                        self.explor_rate = 0.2
                        self.agent_memory = defaultdict(list)
                        self.agent.model = load_model("evaluation_results/baidu_2k_imagenet.h5")
                        self.agent.curr_exploration_rate = 0
                        self.flag = 'DNIM_load'
                        # self.load_model = 1
                    else:
            ...
            #     self.agent.model = load_model("evaluation_results/baidu_2k_FLIR.h5")
            #     return 0, log_dict

            if (recent_reward > self.reward_threshold and recent_acc > self.acc_threshold \
                and self.agent.curr_exploration_rate < 0.1 and self.step_count > 3500) or \
                    (recent_reward > self.reward_threshold and recent_acc > self.acc_threshold \
                     and self.agent.curr_exploration_rate < 0.4 and self.step_count > 9000):
                ...
                if self.step_count > 2000:
                    ...
                    #     # 初始化训练
                    #     self.agent.model.save("evaluation_results/agent_DQN_baidu_FLIR.h5")
                    #     self.agent.model = load_model("evaluation_results/agent_DQN_baidu_FLIR.h5")
                    #     self.flag = 'FLIR'
                    ...
                    #     self.agent.model.save("evaluation_results/agent_DQN_baidu_all_DNIM_retrain.h5")
                    #     self.agent.model = load_model("evaluation_results/agent_DQN_baidu_all_DNIM_retrain.h5")
...
if __name__ == '__main__':
    ...
    running_agent = RunningAgent(  # dqn_path='evaluation_results/agent_DQN_train_baidu_DNIM.h5',
        ...
        dqn_path='evaluation_results/baidu_2k_imagenet.h5',
        banchmark_q=75,
        cloud_backend=api,
    )
    ...
    # FLIR1 = FLIR[:2000]
    # FLIR2 = FLIR1[200:]+FLIR1[:200]
    # np.random.shuffle(FLIR1)
    # test_image_paths = subset + imagenet_paths + subset  # + subset[:300]
    # test_image_paths = subset[:10] #+ imagenet_paths + subset # + subset[:300]
    # test_image_paths = FLIR[:2000]  + imagenet_paths + imagenet_paths[:1000] + FLIR[:2000]# + subset[:300]
    imagenet_paths1 = imagenet_paths
    np.random.shuffle(imagenet_paths1)
    test_image_paths = imagenet_paths + FLIR[:2000] + FLIR[1000:2000] + imagenet_paths1
    # test_image_paths = FLIR[:105]  # + imagenet_paths + subset # + subset[:300]
    # test_image_paths = imagenet_paths  # [:10] #+ imagenet_paths + subset # + subset[:300]
    # + rm.load(7)['image_path'][:1200] + rm.load(7)['image_path']
    ...
    for idx, path in enumerate(test_image_paths):
        ...
        if idx % 10 == 0:
            ...
            #     pickle.dump(ref_cache, f)
            with open('evaluation_results/baidu_all_DNIM_imagenet.defaultdict', 'wb') as f:
                pickle.dump(ref_cache, f)
