...
#     ref_cache = pickle.load(f)
with open('evaluation_results/baidu_all_DNIM.defaultdict', 'rb') as f:
      ref_cache = pickle.load(f)
        ...
        if self.STATUS == "INFERENCE":
            ...
            if len(self.agent_memory['recent_accuracy']) > self.recent_zone and recent_acc < self.acc_threshold:
                if self.load_model == 0:
                    ...
                    self.agent_memory = defaultdict(list)
                    self.agent.model = load_model("evaluation_results/agent_DQN_retrain_baidu_DNIM.h5.retrain.retrain_DNIM1")
                    self.agent.curr_exploration_rate = 0
                    self.load_model = 1
                else:
                    print("infenence to ratrain\n")
...
if __name__ == '__main__':
    ...
    test_image_paths = subset # + subset[:300]#+ imagenet_paths[:600] + subset
    ...
    for idx, path in enumerate(test_image_paths):
        ...
        if idx % 5 == 0:
            ...
            #     pickle.dump(ref_cache, f)
            with open('evaluation_results/baidu_all_DNIM1.defaultdict', 'wb') as f:
                pickle.dump(ref_cache, f)
