...
class RunningAgent(object):
    ...
    def agent_upload(self, image_path):
        ...
        if self.STATUS == "INFERENCE":
            self.explor_rate = 0 #np.clip(self.explor_rate, 0.2, 0.95)
...
if __name__ == '__main__':
    ...
    # test_image_paths = subset + imagenet_paths + subset
    ...
    for idx, path in enumerate(test_image_paths):
        ...
        if idx % 5 == 0:
            ...
            #     pickle.dump(ref_cache, f)
            with open('result/ada_compare_deepn.defaultdict', 'wb') as f:
                pickle.dump(ref_cache, f)
